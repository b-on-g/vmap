namespace $ {
	...
	$mol_test({
		...
		async 'a tree that grew a child paints it without a new frame'( $ ) {
			...
			const node = first.dom_node() as Element
			...
			$mol_assert_equal( ( first.sub()[ 0 ] as $mol_view ).dom_node().isConnected, node.isConnected )
		...
		async 'a literal edited in place repaints nothing by force'( $ ) {
			...
			$mol_assert_equal( ( first.dom_node() as Element ).textContent, 'полка' )
