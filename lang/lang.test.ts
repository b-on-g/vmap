namespace $ {
	...
	$mol_test({
		...
		'a rename leaves the rules of other classes where they were'( $ ) {
			...
			$mol_assert_equal(
				$.$bog_vmap_lang_css_rename( css, `${d}my_site_page`, `${d}my_shop_page` ),
				css,
			)
		...
		'a rename moves the mentions of the class inside a body'( $ ) {
			...
			$mol_assert_equal(
				$.$bog_vmap_lang_js_rename( js, `${d}my_site_page`, `${d}my_shop_page` ),
		...
		'a class renamed and renamed back gives the styles and the body byte for byte'( $ ) {
			...
			$mol_assert_equal(
				$.$bog_vmap_lang_css_rename( $.$bog_vmap_lang_css_rename( css, from, to ), to, from ),
				css,
			)

			$mol_assert_equal(
				$.$bog_vmap_lang_js_rename( $.$bog_vmap_lang_js_rename( js, from, to ), to, from ),
				js,
			)
