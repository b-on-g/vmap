...
namespace $ {
	...
	const over = ( stage: stage, part: string, prop: string, value: string )=> {
		const node = stage.app.node()
		const tree = node.tree()
	...
	const wire = ( stage: stage, from: string, from_port: string, to: string, to_port: string )=> {
		const overlay = stage.overlay()
	...
	$mol_test({
		...
		'the class picker offers one entry per single part item of the shelf'( $ ) {
			...
			for( const klass of offers ) $mol_assert_equal(
				$.$bog_vmap_app_shelf_lone( $.$bog_vmap_app_shelf_single( klass ) ), klass,
			)
